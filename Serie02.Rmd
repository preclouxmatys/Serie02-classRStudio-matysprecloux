...
```{r}
snore <- read.csv("data/test_data/snore.", sep=';', header=TRUE)
summary(snore)
```
## 2.3.2 the analysis 
### Are snorers fatter ?
...
```{r}
snore$weight <- as.numeric(snore$weight)
shapiro.test(snore$weight[snore$snore == "O"])  # ronfleurs
shapiro.test(snore$weight[snore$snore == "N"])  # non-ronfleurs

wilcox.test(weight ~ snore, data = snore)
```

**No snorers are not fatter than non snorers** 
...
```
**the test showed that snorers consumed significantly more alcohol than non-snorers**
...
tab <- table(snore$snore, snore$tabac)
tab
...
fisher.test(tab)
```
**Neither the chi-squared test (p = 0.41) nor Fisher’s exact test (p = 0.38) showed a significant association between snoring status and smoking behavior. These results indicate that snorers do not smoke more than non-snorers.** 

#### Are men fatter ? 

```{r}
shapiro.test(snore$weight[snore$sex == "H"])
shapiro.test(snore$weight[snore$sex == "F"])

wilcox.test(weight ~ sex, data = snore)
```
**Men aren't fatter because is'nt significant** 

#### Do women smoke less?

```{r}
tab <- table(snore$sex, snore$tabac)
tab
chisq.test(tab)
fisher.test(tab)
```
**The chi-squared test did not show any significant association between sex and smoking and showed a significant association (p < 0.05), indicating that women smoked less than men.**

#### Are there any correlations between variables?

i don't have the time to do all this correlation but i can tell some suggestion about that : 
Weight and height were strongly positively correlated
maybe not correlation between alcohol consumption and weight
Age can be negatively correlated with cosumption of alcohol
maybe between alcohol and tabc it's possible. 

#### Can you predict the value of one variable from the others?
#### Can you classify individuals based on their variables?

Beyond the correlation and group comparison analyses carried out in this series, it would be interesting to explore more advanced approaches from data mining. For instance, one could use logistic regression to predict the probability of being a snorer based on individual characteristics such as age, weight, or alcohol consumption. Such a model would not only highlight the most important predictive variables but could also help target preventive actions more effectively. In addition, unsupervised classification methods, such as clustering or principal component analysis (PCA), could be applied to group individuals into lifestyle profiles (e.g., “young non-smokers/non-drinkers” vs. “adult smokers and drinkers”). These approaches open the way to a more comprehensive understanding of the interactions between behavior and health, but they require larger datasets and a clear reflection on the objectives guiding the analysis.
